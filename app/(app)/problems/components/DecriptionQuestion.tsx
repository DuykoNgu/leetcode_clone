      ...
      <div className="scrollbar-hide flex-1 space-y-8 overflow-y-auto px-6 py-6">
        ...
        {constraints.length > 0 && (
          <section className="rounded-2xl border border-slate-100 bg-slate-50/30 p-5">
            <h2 className="mb-4 text-sm font-bold uppercase tracking-wider text-slate-900">Điều kiện (Constraints)</h2>
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-900">Ví dụ (Examples)</h2>
          </div>
          ...
          <div className="space-y-6">
            {examples.map((example, index) => (
              <article key={index} className="group relative rounded-2xl border border-slate-100 bg-slate-50/50 p-5 transition-all hover:bg-white hover:shadow-md">
                <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-400">Ví dụ {index + 1}</h3>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 gap-2">
                    <span className="text-[10px] font-bold uppercase tracking-tight text-slate-400">Đầu vào (Input)</span>
                    <code className="rounded-lg bg-slate-900 px-4 py-3 font-mono text-[13px] text-emerald-400 shadow-inner">
                      {example.input}
                    </code>
                  </div>
                  <div className="grid grid-cols-1 gap-2">
                    <span className="text-[10px] font-bold uppercase tracking-tight text-slate-400">Đầu ra (Output)</span>
