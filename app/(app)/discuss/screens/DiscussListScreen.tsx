              ...
              <div className="flex flex-wrap gap-2">
                {TRENDING_TAGS.map((tag) => (
                  <span 
                    key={tag} 
                    onClick={() => {
                      setSearchQuery(tag);       // Dán từ khóa vào ô input
                      fetchDiscussions(tag);     // 👉 GỌI TÌM KIẾM NGAY LẬP TỨC
                    }} 
                    className="px-3 py-1.5 bg-slate-50 border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700 cursor-pointer rounded-lg text-xs font-semibold text-slate-600 transition-colors"
                  >
                    {tag}
                  </span>
                ))}
